...
superskills 对 HumanEval 一无所知，起作用的是被 hook 强制自动化的「收尾验证」能力（第三轮才做对：纯文字指引被无视两轮，最终靠一个约 100 行的 `stop-verify` hook 确定性强制——改了代码却没运行就拦截一次，要求按文档示例与边界用例真实运行并根因修复）。EvalPlus 专门考察边界用例，提升更明显正说明了这点；HumanEval/154 从 0/3 到 3/3 是最直接的单题证据。诚实记录：HumanEval/101 两臂都是 0——尾随分隔符这个边界，模型的自检枚举没覆盖到，hook 触发了但 bug 仍被放过，这是下一轮要解决的问题。
