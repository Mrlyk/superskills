...
- **S5 澄清·自触发**（`tests/bench/clarify-guidance.sh`）：测 discover 写进 `AGENTS.md` 的那条 clarify 指令本身能否驱动自发澄清。同一模糊任务（"用团队标准化的格式和列导出订单"，引用仓库里不存在的约定），唯一变量是 AGENTS.md 是否带该指令。无指令臂 1/3 主动发问（其余臆测格式），带指令臂 3/3 发问且不写臆测代码——asked 1/3 → 3/3（33%→100%）。同 bench 的"足够具体"任务版两臂都不发问，说明指令不会让模型在具体任务上过度澄清（与公开编码基准零回归一致）。
- **Codex 自动学习**：`codex exec`（ChatGPT 账号默认 gpt-5.5）跑同样的会话回放 + 出厂 LEARN_INSTRUCTION，wiki 合并一次过满分（生成文件、抓两条规则、更新 INDEX、格式、精简全过）。踩坑：守卫文案必须分平台——Codex 文件读写走 shell，"不要跑 shell"会让它直接罢工；Codex 版守卫改为只动 `.superskills/learnings/`、靠 `-s workspace-write` 沙箱兜底。
...
为验证 verify hook 的收益是否泛化并加大样本，引入第二个 EvalPlus 社区基准 **MBPP+**（同维度的单函数编码，题量更大）。同法筛选 0–79 得 10 题困难集，每臂 6 次：
...
## SWE-bench Lite：第三个社区基准（多文件真实仓库）

装好 Docker 后接入第三个、表面上 harness 杠杆最大的社区基准 **SWE-bench Lite**——真实 GitHub issue + 多文件仓库修复，官方 swebench Docker harness 用隐藏的 FAIL_TO_PASS + PASS_TO_PASS 测试评分。

方法学与单函数基准一致，但每实例额外提供**可运行环境**：在 base_commit 检出仓库、装好可编辑的项目与适配版 pytest，让模型能真复现、真跑测试。arm A 纯模型，arm B 把 superskills 完整装进检出的 `.claude/`（4 skill + 3 hook）；模型 patch（git diff，排除 superskills 与 venv 产物）由官方 harness 评分，pass@1。子集为 11 个 gold 验证实例——gold 补丁能在本机 arm64 Docker 复现的题（原拟 12，剔除 pylint×3 实例镜像构建失败、requests×1 测试依赖真实网络，gold 自身都过不了）。

| 臂 | SWE-bench Lite pass@1（11 个 gold 验证实例） |
|----|--------|
| 基线（纯模型） | 6/11（54.5%） |
| 带 superskills | 6/11（**54.5%**） |

**完全持平，且解出的是同一组 6 题**，两臂未解出的也是同一组 5 题。superskills 对「解出哪些」零影响。

机制是干净的：SWE-bench 的验收测试对模型**隐藏**。两臂的 Sonnet 在 agentic 多文件任务上本就会自跑可见测试——实测基线 arm B 的 12 个会话里，模型**自跑 pytest 12/12**，而 `stop-verify.js` 触发 **0/12**（它只在「改了代码却从未执行」时触发，条件根本不满足，验证 hook 在这里全程**休眠**）。它即便触发，也只能强制对可见测试的运行，无法逼近看不见的验收标准。

为实证「无杠杆可提升」，跑了一个完整的优化循环——11 个不同杠杆 + 6 趟方差复现，约 17 次 arm-B 重生成。全 11 题上，已发布配置、注入通用 bug-fix conventions、强制 verify hook 三者得到同一组 6/11。随后聚焦那 5 道两臂都做不出的题，逐杠杆只重生成 arm B（env 已缓存，每轮快）：

| 杠杆（仅注入 arm B，均实验用） | 5 道未解题中解出 |
|------|------|
| conv-tdd（先写复现失败的测试再修） | 1（flask-4992） |
| conv-readtests / conv-plan / conv-edges / conv-combined | 各 0 |
| conv-combined + 强制 verify hook | 1（flask-4992） |
| repro-gate hook（强制写并运行独立复现脚本才收尾） | 0 |
| let-it-cook（TDD + 回合预算 50→80） | 0 |

只有 flask-4992 偶尔翻盘，且只在 9 个杠杆里的 2 个——含相同「先写复现测试」指令的 conv-combined 反而没翻。决定性的方差检验：纯 conv-tdd 在 flask-4992 上复现 **0/3**（连同首次共 1/4），baseline 同题 **0/4**。flask-4992 是一道约 15% 运行间方差的题，那 2 次翻盘是**噪声**、不是杠杆效应。

约 17 次重生成下来，没有任何 superskills 杠杆能稳定提升 SWE-bench 解决率。诚实结论：SWE-bench 上 superskills **结构性持平**。验收标准隐藏时验证 hook 无物可咬；记忆/澄清/discover 规范在单次全新仓库会话里也不适用；瓶颈是「产出正确修复」（那 5 道难题），不是 superskills 提供的能力。这补全了三基准的图景：**增益与「验收标准是否对模型可见、知识是否在仓库代码之外」强相关**——HumanEval+（可见示例 + 隐含边界）+10pp，MBPP+ +3pp，SWE-bench（隐藏验收）持平。整个过程未改动任何已发布配置（conventions 与变体 hook 仅实验用），HumanEval+/MBPP+ 数据按构造不变，无回退。
...
./tests/bench/swe-bench.sh --subset-file tests/bench/swebench/subset-lite.txt   # SWE-bench Lite A/B（第三个社区基准，需 Docker）
```

前置：`claude` CLI 已登录、Node ≥ 18、Python 3；SWE-bench 另需 Docker（守护进程能直连镜像仓库）、`uv`、与一次性 `uv venv` 装好的 `swebench`。Apple Silicon 上 swebench 4.x 硬编码 x86_64、QEMU 模拟极慢，`swebench/swe_eval.py` 把镜像架构改回原生 arm64。重要：`heval-hard.sh` 与 `swe-bench.sh` 运行期间 superskills 插件**不能**装在用户作用域——本地 `directory` marketplace 会把仓库实时文件加载进每一次基线试验、污染 arm A。脚本会给 arm B 自己的 fixture 内 `.claude/`，所以全局安装在这里既不必要也有害。
