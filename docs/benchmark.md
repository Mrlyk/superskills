...
为实证「无杠杆可提升」，跑了两个针对不同机制的优化变体，每轮只重生成 arm B：

| 变体 | SWE-bench Lite pass@1 | 与基线对比 |
|------|--------|--------|
| 已发布 superskills | 6/11 | 持平 |
| + 通用 bug-fix conventions（复现优先、根因、边界、查回归） | 6/11 | 同一组，推理类提示零增益 |
| 强制 verify hook（即便已自测也逼一次「复现 issue + 查回归」，11 题全触发） | 6/11 | 同一组，强制额外验证也零增益 |

四个配置（纯模型、已发布、+conventions、强制 verify）**全部收敛到同一组 6/11**——连在全部 11 题都触发的最强验证杠杆也一题没动。诚实结论：SWE-bench 上 superskills **结构性持平**。验收标准隐藏时验证 hook 无物可咬；记忆/澄清/discover 规范在单次全新仓库会话里也不适用；瓶颈是「产出正确修复」（那 5 道难题），不是 superskills 提供的能力。这补全了三基准的图景：**增益与「验收标准是否对模型可见、知识是否在仓库代码之外」强相关**——HumanEval+（可见示例 + 隐含边界）+10pp，MBPP+ +3pp，SWE-bench（隐藏验收）持平。整个过程未改动任何已发布配置（conventions 与变体 hook 仅实验用），HumanEval+/MBPP+ 数据按构造不变，无回退。
