...
- **S5 澄清·自触发**（`tests/bench/clarify-guidance.sh`）：测 discover 写进 `AGENTS.md` 的那条 clarify 指令本身能否驱动自发澄清。同一模糊任务（"用团队标准化的格式和列导出订单"，引用仓库里不存在的约定），唯一变量是 AGENTS.md 是否带该指令。6 次：无指令臂 4/6 主动发问（其余臆测格式），带指令臂 6/6 发问且不写臆测代码——asked 4/6 → 6/6（67%→100%，**+33pp**）。基线在这种明显不可知的任务上本就常发问，指令的作用是把它拉到可靠的 100%。同 bench 的"足够具体"任务版（如"导出订单历史，让用户下载"）两臂都不发问（0/6 vs 0/6），说明指令不会让模型在具体任务上过度澄清（与公开编码基准零回归一致）。（注：3 次小样本曾给出 1/3→3/3=+67pp，是抖动；6 次的 +33pp 为准——又一次"小样本会骗人"。）
