...
- **S5 clarification · self-triggered** (`tests/bench/clarify-guidance.sh`): tests whether the clarify directive Discover writes into `AGENTS.md` can drive self-clarification on its own. Same ambiguous task ("export orders using the file format and column set our team standardized on" — citing a convention that exists nowhere in the repo); the only variable is whether AGENTS.md carries the directive. Over 6 trials: the no-directive arm asks in 4/6 (guessing the format otherwise); the directive arm asks in 6/6 and writes no premature code — asked 4/6 → 6/6 (67%→100%, **+33pp**). The baseline already asks often on a task this clearly unknowable; the directive's job is to make it reliable (100%). The same bench's "specific-enough" task variant has neither arm asking (0/6 vs 0/6), so the directive does not over-clarify on specific tasks (consistent with zero regression on the public coding benchmarks). (A 3-trial sample earlier read 1/3→3/3 = +67pp; that was noise — the 6-trial +33pp stands, another "small samples lie" reminder.)
- **Codex auto-learning**: `codex exec` (default gpt-5.5 on a ChatGPT account) runs the same session replay + shipped LEARN_INSTRUCTION and does the wiki-merge perfectly on the first try (file generated, both rules captured, INDEX updated, format + concise all pass). Pitfall: guardrail wording must be per-CLI — Codex does file I/O through the shell, so "do not run any shell" makes it refuse outright; the Codex guardrail instead limits edits to `.superskills/learnings/` and leans on the `-s workspace-write` sandbox.
