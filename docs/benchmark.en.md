...
- **S5 clarification · self-triggered** (`tests/bench/clarify-guidance.sh`): tests whether the clarify directive Discover writes into `AGENTS.md` can drive self-clarification on its own. Same ambiguous task ("export orders using the file format and column set our team standardized on" — citing a convention that exists nowhere in the repo); the only variable is whether AGENTS.md carries the directive. The no-directive arm asks in 1/3 (guessing the format otherwise); the directive arm asks in 3/3 and writes no premature code — asked 1/3 → 3/3 (33%→100%). The same bench's "specific-enough" task variant has neither arm asking, so the directive does not over-clarify on specific tasks (consistent with zero regression on the public coding benchmarks).
